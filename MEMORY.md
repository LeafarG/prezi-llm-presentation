...
- **2026-07-08 (morning, focused-mode polish + preview-then-prod workflow)**: Boss asked "O que falta?" and approved 5 fixes for the custos focused-mode UX, then asked for preview before promoting. Workflow: (1) created `fix/focused-mode-polish` branch from `feat/prezi-style`, (2) applied 5 fixes via Node script (content-header, topnav auto-hide, body class toggle, label alignment, data-tour on orcamento), (3) pushed to `prezi` remote as branch (NOT `:main`) — Vercel auto-generated preview URL `prezi-llm-presentation-cfscfj8xz-leafargs-projects.vercel.app`, (4) verified via Puppeteer + screenshot, (5) iterated (topnav at opacity 0.25 + 264px translate was still visible to vision model — made it opacity 0 + 100%+3rem translate in second commit `f6a9383`), (6) boss approved, (7) `git push prezi fix/focused-mode-polish:main` for fast-forward promotion. Vercel created production deployment `prezi-llm-presentation-akdmv1uvk-leafargs-projects.vercel.app` (alias `prezi-llm-presentation.vercel.app`) within 1 second of push. Lesson: **Vercel auto-generates preview URLs for any non-main branch push — to preview-then-promote, push branch (no `:main`) first, verify via screenshot, then fast-forward `:main` after approval. This is the right workflow for boss reviews and should be the default for llm-presentation going forward.** Also: for CSS opacity-based hide tricks, `opacity: 0.25` is still visible to vision models — use `opacity: 0` or `display: none` for "truly hidden" UX.
